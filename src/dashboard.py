...
def cargar_datos():
    ...
    productos["Precio_Unitario"] = (
        productos["Precio_Unitario"].str.replace(",", ".").astype(float)
    )
    ...
    df = df.merge(
        productos[["ID_Producto", "Precio_Unitario"]], on="ID_Producto", how="inner"
    )
    ...
    productos_vendidos = (
        df.groupby("ID_Producto")["Producto_actual_stock"].min().reset_index()
    )
    productos_vendidos = productos_vendidos.merge(
        productos[["ID_Producto", "Stock"]], on="ID_Producto", how="inner"
    )
    productos_vendidos["Cantidad_vendida"] = (
        productos_vendidos["Stock"] - productos_vendidos["Producto_actual_stock"]
    )
...
def preparar_clusters_productos(df, productos_vendidos):
    ...
    cluster_data = productos_vendidos[
        ["ID_Producto", "Cantidad_vendida", "Stock", "Producto_actual_stock"]
    ].copy()
    cluster_data["Porcentaje_venta"] = (
        cluster_data["Cantidad_vendida"] / cluster_data["Stock"]
    ) * 100
    ...
    X_scaled = scaler.fit_transform(
        cluster_data[
            ["Cantidad_vendida", "Stock", "Producto_actual_stock", "Porcentaje_venta"]
        ]
    )
    ...
    cluster_names = {0: "Bajo Rendimiento", 1: "Alto Stock", 2: "Estrella", 3: "Nicho"}
...
def preparar_clusters_clientes(df):
    ...
    cliente_stats_clustering = pd.get_dummies(
        cliente_stats, columns=["region"], drop_first=True
    )
...
def entrenar_modelos(df, cluster_data):
    ...
    df_regression = pd.get_dummies(
        df_regression, columns=["Región", "Categoría"], drop_first=True
    )
    ...
    gb = GradientBoostingRegressor(
        n_estimators=100, learning_rate=0.1, max_depth=4, random_state=12
    )
    ...
    rf = RandomForestRegressor(
        n_estimators=100, max_depth=6, random_state=45, n_jobs=-1
    )
...
def entrenar_skforecast(df_filtrado, forecast_days=30, modelo_tipo="gradient_boosting"):
    ...
    try:
        ...
        ventas_diarias = (
            df_filtrado.groupby("Fecha")["Cantidad_dinero"].sum().reset_index()
        )
        ...
        if modelo_tipo == "gradient_boosting":
            regressor = GradientBoostingRegressor(
                n_estimators=100, max_depth=5, learning_rate=0.1, random_state=72
            )
            modelo_nombre = "Gradient Boosting"
        elif modelo_tipo == "random_forest":
            regressor = RandomForestRegressor(
                n_estimators=100, max_depth=10, random_state=30, n_jobs=-1
            )
        ...
        residuos = (
            ventas_diarias["Cantidad_dinero"].iloc[lags:]
            - forecaster.predict(steps=len(ventas_diarias) - lags).values
        )
...
def entrenar_skforecast_directo(df_filtrado, forecast_days=30):
    ...
    try:
        ...
        forecaster = ForecasterAutoregDirect(
            regressor=Ridge(alpha=1.0), steps=forecast_days, lags=lags
        )
...
def entrenar_prophet_con_regresores(df_filtrado, forecast_days=30):
    ...
    try:
        ...
        # Agregar regresores
        ventas_diarias["es_fin_semana"] = (
            ventas_diarias["ds"].dt.dayofweek.isin([5, 6]).astype(int)
        )
...
def entrenar_holt_winters(df_filtrado, forecast_days=30):
    ...
    try:
        ...
        ventas_diarias.set_index("Fecha", inplace=True)
        ventas_diarias = (
            ventas_diarias.resample("D").sum().fillna(method="ffill").fillna(0)
        )

        # Reemplazar ceros con pequeño valor para evitar errores
        ventas_diarias["Cantidad_dinero"] = ventas_diarias["Cantidad_dinero"].replace(
            0, 0.01
        )
        ...
        seasonal_period = min(
            7, len(ventas_diarias) // 2
        )  # Semanal o menos si hay pocos datos
...
def entrenar_mejor_modelo_temporal(
    df_filtrado, forecast_days=30, modelo_preferido="auto"
):
    """
...
df, productos, productos_vendidos, categorias, clientes_df, metodos_pago = (
    cargar_datos()
)
print("Preparando clusters de productos...")
cluster_productos, kmeans_productos, scaler_productos = preparar_clusters_productos(
...
modelos, feature_cols, X_test, y_test, datos_modelo = entrenar_modelos(
    df, cluster_productos
)
...
def crear_grafica_tendencia_global():
    ...
    ventas_temp["MA7"] = (
        ventas_temp["Cantidad_dinero"].rolling(window=7, min_periods=1).mean()
    )
    ventas_temp["MA30"] = (
        ventas_temp["Cantidad_dinero"].rolling(window=30, min_periods=1).mean()
    )
...
def crear_grafica_regiones():
    ...
    ventas_region = (
        df.groupby("Región")
        .agg({"Cantidad_dinero": "sum", "ID_Venta": "count"})
        .reset_index()
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            marker=dict(
                color=ventas_region["Ingresos"], colorscale="Greens", showscale=False
            ),
...
def crear_grafica_clusters_productos_3d():
    ...
    for cluster_id in sorted(cluster_productos["Cluster"].unique()):
        ...
        nombre = (
            cluster_subset["Cluster_Nombre"].iloc[0]
            if "Cluster_Nombre" in cluster_subset.columns
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                marker=dict(
                    size=6,
                    color=COLORS["clusters"][cluster_id % len(COLORS["clusters"])],
                    opacity=0.8,
                ),
                text=[
                    f"<b>ID: {id}</b><br>Stock: {s}<br>Vendido: {v}<br>% Venta: {p:.1f}%"
                    for id, s, v, p in zip(
                        cluster_subset["ID_Producto"],
...
def crear_grafica_clusters_productos_pie():
    ...
    cluster_dist = (
        cluster_productos.groupby(["Cluster", "Cluster_Nombre"])
        .size()
...
def crear_grafica_radar_productos():
    ...
    cluster_means = cluster_productos.groupby("Cluster")[
        ["Cantidad_vendida", "Stock", "Producto_actual_stock", "Porcentaje_venta"]
    ].mean()
    cluster_means_norm = (cluster_means - cluster_means.min()) / (
        cluster_means.max() - cluster_means.min() + 0.001
    )
    ...
    cluster_nombres = (
        cluster_productos.groupby("Cluster")["Cluster_Nombre"].first().to_dict()
    )
    ...
    for segmento in segmentos_ordenados:
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                text=[
                    f"<b>Cliente: {id}</b><br>Categorías: {c}<br>Frecuencia: {f}<br>Valor: ${m:,.2f}"
                    for id, c, f, m in zip(
                        subset["ID_Cliente"],
                        subset["Categorias"],
                        subset["Frequency"],
...
def crear_grafica_radar_clientes():
    ...
    cluster_means = cluster_clientes.groupby("Segmento")[
        ["Categorias", "Frequency", "Monetary", "Ticket_Promedio"]
    ].mean()
    ...
    for col in cluster_means_norm.columns:
        ...
        cluster_means_norm[col] = (cluster_means[col] - min_val) / (
...
def crear_grafica_valor_cliente_segmento():
    ...
    valor_segmento = (
        cluster_clientes.groupby("Segmento")
        .agg({"Monetary": ["sum", "mean", "count"]})
        .reset_index()
    )
...
def crear_grafica_estacionalidad():
    ...
    ventas_dia = (
        df.groupby("Dia_semana")["Cantidad_dinero"]
        .agg(["sum", "mean", "count"])
        .reset_index()
    )
    ...
    fig = make_subplots(
        rows=1, cols=2, subplot_titles=["Ingresos Totales", "Promedio por Transacción"]
    )
...
def crear_grafica_heatmap():
    ...
    heatmap_data = (
        df.groupby(["Dia_semana", "Mes"])["Cantidad_dinero"].sum().unstack(fill_value=0)
    )
...
def crear_grafica_categorias():
    ...
    ventas_cat = (
        ...
        .agg({"Cantidad_dinero": "sum", "Cantidad": "sum", "ID_Venta": "count"})
        .reset_index()
    )
...
def crear_grafica_metodos_pago():
    ...
    if "Método_Pago" in df.columns:
        ...
        if metodo_stats["Método"].dtype in ["int64", "float64"]:
            metodo_stats["Método"] = (
                metodo_stats["Método"]
                .map(METODOS_PAGO_LABELS)
                .fillna(metodo_stats["Método"].astype(str))
            )
    elif "ID_Metodo" in df.columns:
        metodo_stats = (
            df.groupby("ID_Metodo")
            .agg({"Cantidad_dinero": "sum", "ID_Venta": "count"})
            .reset_index()
        )
        metodo_stats.columns = ["Método", "Ingresos", "Transacciones"]
        ...
        metodo_stats = pd.DataFrame(
            {"Método": ["Sin datos"], "Ingresos": [0], "Transacciones": [0]}
        )
...
def crear_grafica_predicciones_mejor_modelo():
    ...
    fig.update_layout(
        template="plotly_dark",
        title=dict(
            text=f"Predicciones vs Valores Reales ({best_model})", font=dict(size=16)
        ),
...
def crear_grafica_top_predicciones():
    ...
    df_pred = pd.DataFrame(
        {
            ...
            "Error_Pct": np.abs((y_pred_test - y_test.values) / (y_test.values + 0.01))
            * 100,
        }
    )

    df_pred = df_pred.merge(
        productos_info[["ID_Producto", "Categoria", "Segmento"]],
        on="ID_Producto",
        how="left",
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            marker=dict(color="#2ecc71", line=dict(width=1, color="white")),
            ...
            customdata=top_mejor[
                ["ID_Producto", "Categoria", "Segmento", "Valor_Real", "Valor_Predicho"]
            ].values,
            showlegend=False,
        ),
        row=1,
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            marker=dict(color="#e74c3c", line=dict(width=1, color="white")),
    ...
    fig.update_layout(
        template="plotly_dark",
        title=dict(
            text="Productos Mejor y Peor Predichos por el Modelo", font=dict(size=14)
        ),
...
def crear_grafica_predicciones_con_productos():
    ...
    df_pred = pd.DataFrame(
        {
            ...
            "Error_Pct": np.abs((y_test.values - y_pred_test) / (y_test.values + 0.01))
            * 100,
        }
    )
    ...
    df_pred = df_pred.merge(
        productos_info[["ID_Producto", "Categoria", "Segmento", "Precio"]],
        on="ID_Producto",
        how="left",
    )
    ...
    for cluster_nombre in CLUSTER_PRODUCTO_COLORS.keys():
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    color=color, size=10, opacity=0.7, line=dict(width=1, color="white")
                ),
                customdata=subset[
                    [
                        "ID_Producto",
...
def crear_grafica_error_por_segmento():
    ...
    colors_seg = [
        CLUSTER_PRODUCTO_COLORS.get(seg, "#888") for seg in stats_segmento["Segmento"]
    ]
    ...
    fig.update_layout(
        template="plotly_dark",
        title=dict(
            text="Analisis de Precision por Cluster (Conjunto de Prueba)",
            font=dict(size=16),
        ),
        margin=dict(l=20, r=20, t=70, b=20),
        height=380,
...
def crear_grafica_top_productos():
    ...
    top_productos = (
        df.groupby("ID_Producto")["Cantidad_dinero"].sum().nlargest(10).reset_index()
    )
...
def crear_grafica_top_clientes():
    ...
    top_clientes = (
        df.groupby("ID_Cliente")["Cantidad_dinero"].sum().nlargest(10).reset_index()
    )
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        crear_kpi_card(
                            "Ingresos Totales", f"${total_ventas:,.2f}", "", "success"
                        )
                    ],
                    width=2,
                ),
                dbc.Col(
                    [
                        crear_kpi_card(
                            "Unidades Vendidas", f"{total_unidades:,}", "", "success"
                        )
                    ],
                    width=2,
                ),
                dbc.Col(
                    [
                        crear_kpi_card(
                            "Ticket Promedio", f"${ticket_promedio:.2f}", "", "success"
                        )
                    ],
                    width=2,
                ),
                dbc.Col(
                    [
                        crear_kpi_card(
                            "Productos", f"{productos_unicos:,}", "", "success"
                        )
                    ],
                    width=2,
                ),
                dbc.Col(
                    [crear_kpi_card("Clientes", f"{clientes_unicos:,}", "", "success")],
                    width=2,
                ),
                dbc.Col(
                    [
                        crear_kpi_card(
                            "Regiones", f"{regiones_activas:,}", "", "success"
                        )
                    ],
                    width=2,
                ),
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.P(
                            "Stock, Cantidad Vendida, Stock Actual y Porcentaje de Rotación",
                                                            ...
                                                            [
                                                                html.Div(
                                                                    [
                                                                        ...
                                                                        html.Br(),
                                                                        html.Small(
                                                                            f"{len(cluster_productos[cluster_productos['Cluster'] == i])} productos",
                                                            ...
                                                            [
                                                                html.Tr(
                                                                    [
                                                                        ...
                                                                        html.Td(
                                                                            f"{len(cluster_productos[cluster_productos['Cluster_Nombre'] == nombre])}"
                                                                        ),
                                                                        html.Td(
                                                                            f"{cluster_productos[cluster_productos['Cluster_Nombre'] == nombre]['Cantidad_vendida'].mean():.0f}"
                                                                        ),
                                                                        html.Td(
                                                                            f"{cluster_productos[cluster_productos['Cluster_Nombre'] == nombre]['Porcentaje_venta'].mean():.1f}%"
                                                                        ),
                                                                    ]
                                                                )
                                                                for nombre in cluster_productos[
                                                                    "Cluster_Nombre"
                                                                ].unique()
                                                            ]
                                                        ),
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H5(
                                            "Descripción de Segmentos de Productos",
                                            className="card-title mb-4",
                                        ),
                                        dbc.Row(
                                            [
                                                    ...
                                                    [
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Alta rotación y buenas ventas. Productos más rentables del inventario. Mantener stock óptimo y priorizar en promociones.",
                                                        ...
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.Strong("Nicho"),
                                                                html.P(
                                                                    "Stock limitado pero demanda específica. Productos especializados con mercado definido. Monitorear para evitar quiebres de stock.",
                                                                    className="text-muted small mb-3",
                                                                ),
                                                            ]
                                                        ),
                                                    ...
                                                    [
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Mucho inventario con rotación moderada. Capital inmovilizado. Aplicar descuentos o promociones para acelerar ventas.",
                                                                    className="text-muted small mb-3",
                                                                ),
                                                            ]
                                                        ),
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Baja rotación y pocas ventas. Evaluar discontinuar o liquidar. Liberar espacio para productos más rentables.",
                                                                    className="text-muted small mb-3",
                                                                ),
                                                            ]
                                                        ),
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.P(
                            "Frecuencia de compras, total comprado, gasto total, caterogias distintas, promedio de gasto de compra, región ",
                            className="text-center text-muted mb-4",
                        ),
                        html.Hr(),
                    ]
                                    ...
                                    [
                                        ...
                                        dbc.Row(
                                            [
                                                    ...
                                                    [
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.Br(),
                                                                html.Small(
                                                                    f"{len(cluster_clientes[cluster_clientes['Segmento'] == segmento])} clientes",
                                                                    className="text-muted",
                                                                ),
                                                                html.Br(),
                                                                html.Small(
                                                                    f"${cluster_clientes[cluster_clientes['Segmento'] == segmento]['Monetary'].sum():,.0f}",
                                                ...
                                                for segmento in SEGMENTO_COLORS.keys()
                                                if len(
                                                    cluster_clientes[
                                                        cluster_clientes["Segmento"]
                                                        == segmento
                                                    ]
                                                )
                                                > 0
                                                    ...
                                                    [
                                                        html.Tr(
                                                            [
                                                                ...
                                                                html.Td(
                                                                    f"{len(cluster_clientes[cluster_clientes['Segmento'] == segmento]):,}"
                                                                ),
                                                                html.Td(
                                                                    f"{cluster_clientes[cluster_clientes['Segmento'] == segmento]['Categorias'].mean():.1f}"
                                                                ),
                                                                html.Td(
                                                                    f"{cluster_clientes[cluster_clientes['Segmento'] == segmento]['Frequency'].mean():.1f}"
                                                                ),
                                                                html.Td(
                                                                    f"${cluster_clientes[cluster_clientes['Segmento'] == segmento]['Ticket_Promedio'].mean():,.0f}"
                                                                ),
                                                                html.Td(
                                                                    f"${cluster_clientes[cluster_clientes['Segmento'] == segmento]['Monetary'].sum():,.0f}",
                                                    ...
                                                    [
                                                        html.Div(
                                                            [
                                                                html.Span(
                                                                    "●",
                                                                    style={
                                                                        "color": SEGMENTO_COLORS[
                                                                            "Premium y Frecuentes"
                                                                        ],
                                                                        "marginRight": "8px",
                                                                    },
                                                                ),
                                                                ...
                                                                html.P(
                                                                    "Clientes de alto valor, compran seguido y variado. Programas de lealtad premium.",
                                                        ...
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Compran menos pero enfocados en categorías particulares. Ofertas específicas.",
                                                                    className="text-muted small mb-2",
                                                                ),
                                                            ]
                                                        ),
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Regulares en gasto y frecuencia. Segmento medio. Recompensas por constancia.",
                                                                    className="text-muted small mb-2",
                                                                ),
                                                            ]
                                                        ),
                                                    ...
                                                    [
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Compran con poca frecuencia pero variado. Promociones estacionales.",
                                                                    className="text-muted small mb-2",
                                                                ),
                                                            ]
                                                        ),
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Compra orientada a bajo costo. Cupones y flash sales.",
                                                                    className="text-muted small mb-2",
                                                                ),
                                                            ]
                                                        ),
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Ticket alto pero poca variedad. Emails de bienvenida o reactivación.",
                                                                    className="text-muted small mb-2",
                                                                ),
                                                            ]
                                                        ),
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H3(
                            "Modelos Predictivos de Ingresos por Producto",
                            className="text-center mb-4 mt-3",
                        ),
                        html.P(
                            "Precio unitario, Stock, Stock Actual, Porcentaje de venta, Región, Categoria ",
                            className="text-center text-muted mb-4",
                        ),
                        html.Hr(),
                    ]
                            ...
                            [
                                dbc.CardBody(
                                    [
                                        ...
                                        html.H3(
                                            best_model, className="text-success mb-0"
                                        ),
                                    ]
                                )
                            ],
                            ...
                            [
                                dbc.CardBody(
                                    [
                                        ...
                                        html.H3(
                                            f"{modelos[best_model]['metrics']['R2']:.4f}",
                                            className="text-info mb-0",
                                        ),
                                        html.Small(
                                            f"Explica {modelos[best_model]['metrics']['R2'] * 100:.1f}% de la varianza",
                                            className="text-muted",
                                        ),
                                    ]
                                )
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H6(
                                            "RMSE", className="card-title text-muted"
                                        ),
                                        html.H3(
                                            f"{modelos[best_model]['metrics']['RMSE']:,.0f}",
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H6(
                                            "MAE", className="card-title text-muted"
                                        ),
                                        html.H3(
                                            f"{modelos[best_model]['metrics']['MAE']:,.0f}",
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        dcc.Graph(
                                            figure=fig_comparacion_modelos,
                                            config={"displayModeBar": False},
                                        )
                                    ]
                                )
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H5(
                                            "Productos Mejor y Peor Predichos",
                                            className="card-title mb-2",
                                        ),
                                        html.P(
                                            "Identifica donde el modelo acierta y donde falla",
                                            className="text-muted small",
                                        ),
                                        dcc.Graph(
                                            figure=fig_top_predicciones_modelo,
                                            config={"displayModeBar": False},
                                        ),
                                    ]
                                )
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H5(
                                            "Consultar Prediccion de un Producto",
                                            className="card-title mb-3",
                                        ),
                                        ...
                                        dbc.Row(
                                            [
                                                ...
                                                dbc.Col(
                                                    [
                                                        html.Div(
                                                            id="resultado-prediccion-producto",
                                                            className="mt-2",
                                                        )
                                                    ],
                                                    width=6,
                                                        ...
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.Ul(
                                                                    [
                                                                        html.Li(
                                                                            "Estimar ingresos de nuevos productos",
                                                                            className="small",
                                                                        ),
                                                                        html.Li(
                                                    ...
                                                    [
                                                        html.Tr(
                                                            [
                                                                ...
                                                                html.Td(
                                                                    f"{modelos[modelo]['metrics']['R2']:.4f}"
                                                                ),
                                                                html.Td(
                                                                    f"{modelos[modelo]['metrics']['RMSE']:,.0f}"
                                                                ),
                                                                html.Td(
                                                                    f"{modelos[modelo]['metrics']['MAE']:,.0f}"
                                                                ),
                                                                html.Td(
                                                                    dbc.Badge(
                                                                        "Mejor",
                            ...
                            [
                                dbc.CardBody(
                                    [
                                                    ...
                                                    [
                                                        ...
                                                        dcc.Dropdown(
                                                            ...
                                                            options=[
                                                                ...
                                                                {
                                                                    "label": "Prophet (Regresores)",
                                                                    "value": "prophet_reg",
                                                                },
                                                                {
                                                                    "label": "ARIMA (Estadistico)",
                                                                    "value": "arima",
                                                                },
                                                            ],
                                                            value="prophet_default",
                                        ...
                                        html.Div(
                                            id="info-modelo-forecast", className="mt-2"
                                        ),
                                    ]
                                )
                            ],
...
if "Premium y Frecuentes" in cluster_clientes["Segmento"].values:
    valor_premium = cluster_clientes[
        cluster_clientes["Segmento"] == "Premium y Frecuentes"
    ]["Monetary"].sum()
                            ...
                            [
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "Se identificaron 4 segmentos de productos. Los productos 'Estrella' representan las mejores oportunidades de negocio con alta rotación."
                                        ),
                                    ],
                                    color="info",
                                    className="mb-3",
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            f"El análisis reveló 6 segmentos de clientes. Los clientes 'Premium y Frecuentes' generan ${valor_premium:,.0f} en valor total."
                                        ),
                                    ],
                                    color="success",
                                    className="mb-3",
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "El modelo Gradient Boosting logró un R² de 0.8476, permitiendo predecir ingresos con alta precisión."
                                        ),
                                    ],
                                    color="warning",
                                    className="mb-3",
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "El negocio generó $103,947.36 en ingresos totales, con 3,000 transacciones de 326 clientes únicos."
                                        ),
                                    ],
                                    color="primary",
                                    className="mb-3",
                            ...
                            [
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "Detectar razones de compra, satisfacción y promociones preferidas."
                                        ),
                                    ],
                                    color="light",
                                    className="mb-3",
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "Clima, días festivos, eventos o redes sociales podrían impactar ventas."
                                        ),
                                    ],
                                    color="light",
                                    className="mb-3",
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "Usar reseñas, atención al cliente y menciones en redes para conocer percepciones."
                                        ),
                                    ],
                                    color="light",
                                    className="mb-3",
                                ...
                                dbc.Alert(
                                    [
                                        ...
                                        html.P(
                                            "Analizar tiempo entre compras y productos relacionados."
                                        ),
                                    ],
                                    color="light",
                                    className="mb-3",
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H4(
                                            " Estrategias por Segmento de Clientes",
                                            className="card-title mb-3",
                                        ),
                                        dbc.Table(
                                            [
                                                ...
                                                html.Tbody(
                                                    [
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 0 – Premium y Frecuentes"
                                                                ),
                                                                html.Td(
                                                                    "Compran mucho, seguido y en muchas categorías."
                                                                ),
                                                                html.Td(
                                                                    "Programa de fidelización exclusivo y beneficios premium."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 1 – Exploradores de Nicho"
                                                                ),
                                                                html.Td(
                                                                    "Compran en categorías específicas."
                                                                ),
                                                                html.Td(
                                                                    "Promociones focalizadas para explorar nuevas categorías."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 2 – Clientes Estables"
                                                                ),
                                                                html.Td(
                                                                    "Consistentes en gasto y frecuencia."
                                                                ),
                                                                html.Td(
                                                                    "Recompensas por constancia y muestras para ampliar interés."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 3 – Ocasionales Económicos"
                                                                ),
                                                                html.Td(
                                                                    "Compran poco pero variado."
                                                                ),
                                                                html.Td(
                                                                    "Campañas de recurrencia y packs por categoría."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 4 – Cazadores de Oferta"
                                                                ),
                                                                html.Td(
                                                                    "Muy sensibles al precio."
                                                                ),
                                                                html.Td(
                                                                    "Cupones, descuentos flash y combos tipo “lleva más por menos”."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 5 – Nuevos o Dormidos"
                                                                ),
                                                                html.Td(
                                                                    "Compran poco o están inactivos."
                                                                ),
                                                                html.Td(
                                                                    "Campañas de reactivación o bienvenida."
                                                                ),
                                                            ]
                                                        ),
                                                    ]
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H4(
                                            "Estrategias por Segmento de Productos",
                                            className="card-title mb-3",
                                        ),
                                        dbc.Table(
                                            [
                                                    ...
                                                    [
                                                        html.Tr(
                                                            [
                                                                ...
                                                                html.Td(
                                                                    "Baja rotación y pocas ventas."
                                                                ),
                                                                html.Td(
                                                                    "Evaluar discontinuar o liquidar. Liberar espacio para productos más rentables."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                html.Td(
                                                                    "Cluster 1 – Alto Stock"
                                                                ),
                                                                html.Td(
                                                                    "Mucho inventario con rotación moderada."
                                                                ),
                                                                html.Td(
                                                                    "Aplicar descuentos o promociones para acelerar ventas."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                ...
                                                                html.Td(
                                                                    "Alta rotación y buenas ventas.."
                                                                ),
                                                                html.Td(
                                                                    "Mantener niveles y promover cross-selling."
                                                                ),
                                                            ]
                                                        ),
                                                        html.Tr(
                                                            [
                                                                ...
                                                                html.Td(
                                                                    "Stock limitado pero demanda específica."
                                                                ),
                                                                html.Td(
                                                                    " Monitorear para evitar quiebres de stock."
                                                                ),
                                                            ]
                                                        ),
                                                    ]
                                            ...
                                            [
                                                html.Div(
                                                    [
                                                        ...
                                                        html.P(
                                                            "Reducir stock de productos de 'Bajo Rendimiento' y aumentar productos 'Estrella'.",
                                                            className="text-muted small",
                                                        ),
                                                    ],
                                                ...
                                                html.Div(
                                                    [
                                                        ...
                                                        html.P(
                                                            "Crear programa VIP para retener clientes 'Premium y Frecuentes' y recuperar 'Nuevos o Dormidos'.",
                                                            className="text-muted small",
                                                        ),
                                                    ],
                                                ...
                                                html.Div(
                                                    [
                                                        ...
                                                        html.P(
                                                            "Diseñar promociones específicas para cada segmento de cliente según su comportamiento.",
                                                            className="text-muted small",
                                                        ),
                                                    ],
                                                ...
                                                html.Div(
                                                    [
                                                        ...
                                                        html.P(
                                                            "Enfocar esfuerzos en las regiones de mayor potencial identificadas en el análisis.",
                                                            className="text-muted small",
                                                        ),
                                                    ],
                                                ...
                                                html.Div(
                                                    [
                                                        ...
                                                        html.P(
                                                            "Implementar el modelo predictivo para anticipar demanda y optimizar operaciones.",
                                                            className="text-muted small",
                                                        ),
                                                    ],
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H4(
                                            "Resumen de Métricas del Análisis",
                                            className="card-title mb-4",
                                        ),
                                        dbc.Row(
                                            [
    ...
    [
        ...
        dbc.Tabs(
            [
                ...
                dbc.Tab(
                    tab_conclusiones, label="Conclusiones", tab_id="tab-conclusiones"
                ),
            ],
            id="tabs",
...
def predecir_producto(id_producto):
    ...
    if id_producto is None:
        return html.Div(
            [
                html.P(
                    "Selecciona un producto para ver su prediccion",
                    className="text-muted",
                )
            ]
        )
    ...
    if not mask_test.any():
        return dbc.Alert(
            "Producto no encontrado en conjunto de prueba", color="warning"
        )
                                ...
                                [
                                    html.Div(
                                        [
                                            ...
                                            html.Small(
                                                "Ingreso Real", className="text-muted"
                                            ),
                                        ],
                                        className="text-center",
                                    )
                                ...
                                [
                                    html.Div(
                                        [
                                            ...
                                            html.Small(
                                                "Prediccion", className="text-muted"
                                            ),
                                        ],
                                        className="text-center",
                                    )
...
@callback(
    [Output("grafico-forecast", "figure"), Output("info-modelo-forecast", "children")],
    [
...
def actualizar_forecast(forecast_days, region, modelo_seleccionado):
    ...
    resultado, error = entrenar_mejor_forecast(
        df_filtrado, forecast_days, modelo_seleccionado
    )
    ...
    if hist_col == "y":
        hist_values = (
            resultado["historico"]["y"]
            if "y" in resultado["historico"].columns
            else resultado["historico"].iloc[:, 0]
        )
    else:
    ...
    fig.add_trace(
        go.Scatter(
            x=list(resultado["forecast_index"])
            + list(resultado["forecast_index"][::-1]),
            y=list(resultado["conf_int_upper"])
            + list(resultado["conf_int_lower"][::-1]),
            fill="toself",
    ...
    fig.update_layout(
        template="plotly_dark",
        title=dict(
            text=f"Pronóstico {model_name} - {forecast_days} días", font=dict(size=16)
        ),
        xaxis_title="Fecha",
        yaxis_title="Ingresos ($)",
        height=400,
    ...
    if "order" in resultado:
        info_items.append(
            dbc.Badge(f"Order: {resultado['order']}", color="info", className="me-2")
        )
    if "aic" in resultado:
        info_items.append(
            dbc.Badge(
                f"AIC: {resultado['aic']:.1f}", color="secondary", className="me-2"
            )
        )
                    ...
                    dbc.Row(
                        [
                            dbc.Col(
                                [
                                    html.Small(
                                        "Promedio diario:", className="text-muted"
                                    ),
                                    html.Span(
                                        f" ${forecast_mean:,.2f}",
...
if __name__ == "__main__":
    ...
    print(
        f"- Mejor modelo: {best_model} (R²={modelos[best_model]['metrics']['R2']:.4f})"
    )
    print("=" * 60)
